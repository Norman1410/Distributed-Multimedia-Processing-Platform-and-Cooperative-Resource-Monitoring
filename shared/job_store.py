...
class JobStore:
    ...
    def create_job(
        ...
        with self._connection() as conn:
            conn.execute(
                ...
                """,
                (job_id, file_path, operation, priority, "pending", 0, now, now),
            )
    ...
    def mark_job_failed(self, job_id: str, error_message: str) -> Optional[Dict[str, Any]]:
    ...
    def record_job_result(
        ...
        with self._connection() as conn:
            ...
            conn.execute(
                """
                UPDATE jobs
                SET result_path = ?, status = ?, progress = ?, finished_at = ?, updated_at = ?
                WHERE job_id = ?;
                """,
